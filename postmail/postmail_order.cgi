...
sub prev_form {
	...
	# テンプレート読込
	open(IN,"$cf{tmpldir}/conf_order.html") or error("open err: conf_order.html");
	my $tmpl = join('',<IN>);
	close(IN);
...
sub send_mail {
	...
	if ($cf{auto_res}) {
		# テンプレ
		open(IN,"$cf{tmpldir}/reply_order.txt") or error("open err: reply_order.txt");
		$reply = join('',<IN>);
		close(IN);
	...
	# 完了メッセージ
		...
		open(IN,"$cf{tmpldir}/thanks_order.html") or error("open err: thanks_order.html");
		my $tmpl = join('',<IN>);
		close(IN);
...
sub err_input {
	...
	# テンプレート読み込み
	open(IN,"$cf{tmpldir}/error_order.html") or die;
	my $tmpl = join('',<IN>);
	close(IN);
