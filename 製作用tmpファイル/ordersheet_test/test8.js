...
function createUnitPrice(unit_num){
  ...
  for (let target of size_radio_btns) {
    target.addEventListener(`change`, () => {
      ...
      addUnitPriceObj(unit_num, unit_price);
      updateTotalPrice(calclateTotalPrice());
       //console.log(calclateTotalPrice());
    });
  }
...
//単価オブジェクトに追加
function addUnitPriceObj(unit_num, unit_price){
  unit_price_obj[unit_num] = unit_price;
  //console.log(Object.values(unit_price_obj));
  console.log(unit_price_obj); //あとで削除
}

//単価オブジェクトから削除
function deleteUnitPriceObj(unit_num){
  delete unit_price_obj[unit_num];
  console.log(unit_price_obj); //あとで削除
}
...
function calclateTotalPrice(){
  total_price = 0;
  Object.values(unit_price_obj).forEach(value =>{
    total_price += value;
  });
  return total_price;
}
